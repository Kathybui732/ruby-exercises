...
class NestedTest < MiniTest::Test
  ...
  def test_list_of_employees_across_all_restaurants
    ...
    # employee_names = []
    # stores.each do |stores, aspects|
    #   employee_names << aspects[:employees]
    # end
    # employee_names.flatten!

    employee_names = stores.map do |store, data|
      data[:employees]
    end
    employee_names.flatten!
  ...
  def test_full_menu_price_for_olive_garden
    ...
    #=======================
  #   full_menu_price = 0
  #   stores[:olive_garden][:dishes].each do |value|
  #     full_menu_price = full_menu_price + value[:price]
  #   end
  # end

    full_menu_price = stores[:olive_garden][:dishes].reduce(0) do |sum, value|
      sum + value[:price]
    end
    #=======================
  ...
  def test_menu_accross_all_restaurants
    ...
    #=======================
    full_menu = Hash.new
    stores.each do |store, aspect|
      aspect[:dishes].each do |dish|
